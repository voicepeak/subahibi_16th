...
const threadMeta = [
  {
    id: "shiroyama-fall",
    dateRange: "2012/07/10 - 2012/07/11",
    summary: "城山翼坠楼后，匿名讨论从轻率旁观滑向个人信息曝光与最初的预言。",
    dangerLevel: 3 as const,
  },
  {
    id: "big-hazard",
    dateRange: "2012/07/11 - 2012/07/12",
    summary: "高岛柘榴把灾难、封印与世界终结写成一套自洽的电波神话。",
    dangerLevel: 4 as const,
  },
  {
    id: "strange-mail",
    dateRange: "2012/07/12 - 2012/07/13",
    summary: "第一批奇怪短信出现，恐惧从帖子扩散到每个人的手机。",
    dangerLevel: 4 as const,
  },
  {
    id: "takashima-confirmed",
    dateRange: "2012/07/13 - 2012/07/15",
    summary: "确认帖把零散证言整理成档案，短信倒计时变得可追踪。",
    dangerLevel: 5 as const,
  },
  {
    id: "savior",
    dateRange: "2012/07/15 - 2012/07/16",
    summary: "救世主的发言接管了恐惧，掲示板变成仪式的回放装置。",
    dangerLevel: 5 as const,
  },
] as const;

function scorePost(post: LegacyPost, dangerLevel: 1 | 2 | 3 | 4 | 5) {
  const text = `${post.name} ${post.uid} ${post.body}`;
  const highlight = /高岛|柘榴|间宫|救世主|世界|终结|死|短信|诅咒|Big hazard/i.test(text);
  return {
    highlight,
    fearScore: Math.min(5, dangerLevel + (highlight ? 1 : 0)) as 1 | 2 | 3 | 4 | 5,
    rumorScore: Math.min(5, Math.max(1, dangerLevel - 1 + (/听说|好像|应该|预言|确认/.test(text) ? 1 : 0))) as 1 | 2 | 3 | 4 | 5,
  };
}

export const bulletinThreads: BulletinThread[] = legacyThreads.map((thread, threadIndex) => {
  const meta = threadMeta[threadIndex];
  return {
    id: meta.id,
    title: thread.title,
    ...
    posts: thread.posts.map((post, postIndex) => ({
      id: `${meta.id}-${post.num}-${postIndex}`,
      no: post.num,
      author: post.name,
      authorId: post.uid,
      datetime: post.date,
      content: post.body,
      tags: post.uid === "zakuro" || post.uid === "mamiya" ? ["关键ID"] : undefined,
      ...scorePost(post, meta.dangerLevel),
    })),
  };
});

export const bulletinTimeline = bulletinThreads.flatMap((thread) =>
  thread.posts
    ...
    .map((post) => ({
      ...
      label: `No.${post.no}`,
      excerpt: post.content.slice(0, 42),
      fearScore: post.fearScore ?? thread.dangerLevel,
    }))
);
