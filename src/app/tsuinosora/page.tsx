...
const SKY_CHAPTERS = [
  {
    title: "序",
    subtitle: "我们的情人",
    bg: "/assets/cg/main/ev0009.png",
    quote: "我们的情人，不过是随便借个名字，用幻想吹出来的肥皂泡。",
    source: "埃德蒙·罗斯坦《西哈诺·德·贝热拉克》",
    text: "那片天空之下。这个世界存在于此。",
  },
  {
    title: "壹",
    subtitle: "天空的起始",
    bg: "/assets/bg/bg1015a.png",
    quote: "世界是我所看到、触摸到、感受到的东西。所以，世界和我没有差别。",
    source: "水上由岐",
    text: "由岐望着天空。神并不存在——但她说出了那个词。",
  },
  {
    title: "贰",
    subtitle: "夏夜大三角",
    bg: "/assets/bg/bg1015d.png",
    quote: "在夏日中闪耀的大三角……无限的线即三角形……",
    source: "高岛柘榴",
    text: "顶点织女、牛郎、天津四。在星空下，少女们寻找着归还天空的地点。",
  },
  {
    title: "叁",
    subtitle: "黄昏之时",
    bg: "/assets/bg/bg1015b.png",
    quote: "我们的头脑比天空更辽阔——来，将二者对比着看一看。",
    source: "艾米莉·狄金森",
    text: "意识能容纳整个天空。那么，天空的极限在哪里？世界的极限又在哪里？",
  },
  {
    title: "肆",
    subtitle: "世界终结",
    bg: "/assets/denpa/sp0040a.png",
    quote: "世界要终结了。必然要终结了。这是真实。",
    source: "间宫卓司",
    text: "救世主宣告了末日的预言。回归天空的日子是7月20日。",
  },
  {
    title: "伍",
    subtitle: "坠落",
    bg: "/assets/denpa/sp0012a.png",
    quote: "月正笑。仿佛一无所知般。仿佛对人间的一切丑恶都浑然不知。",
    source: "间宫皆守",
    text: "从屋顶坠落——那一瞬间，时间慢了下来。",
  },
  {
    title: "陆",
    subtitle: "偏在转生",
    bg: "/assets/denpa/sp0008g.png",
    quote: "全部的'我'——不管是那个丑陋的女孩，还是那个漂亮的孩子。他们全部都是'我'。",
    source: "音无彩名 · 假设7",
    text: "只有一个灵魂。它轮回于所有生命之中。",
  },
  {
    title: "柒",
    subtitle: "向日葵的坡道",
    bg: "/assets/denpa/sp0084a.png",
    quote: "没有必要给死者供花了。",
    source: "水上由岐",
    text: "在坡道的顶端，天空依然广阔。什么都不曾改变，一切都已改变。",
  },
  {
    title: "终",
    subtitle: "终之空",
    bg: "/assets/denpa/sp0001h.png",
    quote: "幸福地生活吧。",
    source: "路德维希·维特根斯坦《草稿》",
    text: "这便是世界的全部意义。",
  },
];
...
function SkyChapter({ chapter, index, glitching }: { chapter: typeof SKY_CHAPTERS[0]; index: number; glitching: boolean }) {
  ...
  return (
    <section ref={ref} className={`sky-chapter${revealed ? " sky-chapter-in" : ""}`}>
      <div className="sky-chapter-bg">
        <img src={chapter.bg} alt="" className="sky-chapter-img" />
        <div className="sky-chapter-overlay" />
      </div>
      <div className={`sky-chapter-content${isLast ? " sky-chapter-last" : ""}`}>
        <div className={`sky-chapter-badge${revealed ? " sky-chapter-burst" : ""}`}>
...
function GlitchIntro({ onDone }: { onDone: () => void }) {
  ...
  return (
    <>
      ...
      {/* RGB bursts - intermittent and aggressive */}
      {rgbBurst && (
        <>
      ...
      {/* Static RGB overlay during intense phases */}
      ...
      {/* CRT static bands - like old TV interference */}
      {phase >= 3 && (
        <>
      ...
      {/* Text overlays with corruption - hidden during final flicker */}
      {showText >= 0 && showText < texts.length && phase < 37 && (
        <div className={`glitch-text${rgbBurst ? " glitch-text-corrupt" : ""}`}>
          {texts[showText]}
        </div>
      )}
      ...
      {/* Horizontal bar interference */}
      {phase >= 6 && (
        <div className="glitch-bars" />
      )}

      {/* Digital artifact blocks - more during rgb bursts */}
      {rgbBurst && (
        <>
...
export default function TsuiNoSoraPage() {
  ...
  useEffect(() => {
    ...
    let timerRef: ReturnType<typeof setTimeout> = scheduleNext();
